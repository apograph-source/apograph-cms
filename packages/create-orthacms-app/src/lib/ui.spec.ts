...
describe('the keyboard pickers', () => {
    ...
    describe('a locked row', () => {
        it('is in the answer with nothing pressed [create-orthacms-app:I-10]', async () => {
        ...
        it('does not take the cursor, so the space bar lands elsewhere [create-orthacms-app:I-10]', async () => {
            const answer = multiselect('Which protocols?', PROTOCOLS);
        ...
        it('cannot be unticked by the space bar [create-orthacms-app:I-10]', async () => {
            const answer = multiselect('Which protocols?', PROTOCOLS);
     ...
     */
    describe('raw mode', () => {
        it('is left as it was found once a picker is answered [create-orthacms-app:I-21]', async () => {
            const answer = multiselect('Which backends?', COPILOT);
            await type(SPACE);
        ...
        it('is left as it was found when the picker is cancelled [create-orthacms-app:I-21]', async () => {
            const answer = select('Where?', MEDIA);
            await type(ESC);
        ...
        it('is left as it was found when stdin errors [create-orthacms-app:I-21]', async () => {
            const answer = select('Where?', MEDIA);
            await new Promise((resolve) => setImmediate(resolve));
