...
export const USAGE = `create-orthacms-app — scaffold an Ortha CMS app

Usage: npx create-orthacms-app <directory> [options]
 ...
 * resolving `latest` from the registry also means `npx create-orthacms-app@0.3.0`
...
export async function resolveAnswers(
    ...
    const defaultDatabaseUrl = `postgresql://orthacms:orthacms@localhost:5432/${defaultName.replace(
        /[^a-z0-9_]/gi,
        '_'
    )}`;
...
function describeSelection(
    ...
    return [
        ...
        ['Ortha CMS packages', String(resolvePackages(selection).length + 1)]
    ];
}
