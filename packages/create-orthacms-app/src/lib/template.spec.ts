...
beforeEach(() => {
    target = mkdtempSync(join(tmpdir(), 'create-orthacms-'));
});
...
describe('the scaffolded app, whatever the features', () => {
    ...
    it('pins every @orthacms dependency to this scaffolder’s version [create-orthacms-app:I-03]', () => {
        const { dependencies, devDependencies } = manifest();
        const orthacms = Object.entries({
            ...dependencies,
            ...devDependencies
        }).filter(([name]) => name.startsWith('@orthacms/'));

        expect(orthacms.length).toBeGreaterThan(0);
        for (const [, range] of orthacms) expect(range).toBe('9.9.9');
    });
    ...
    it('keeps the non-Ortha CMS dependencies the template declares', () => {
     ...
     */
    it('restores the dotfiles npm will not publish [create-orthacms-app:I-22]', () => {
    ...
    it('renames the .tmpl files onto their real names [create-orthacms-app:I-22]', () => {
    ...
    it('scaffolds no signing secrets, because identity has none to sign with [create-orthacms-app:I-17]', () => {
     ...
     */
    it('leaves no placeholder or directive anywhere in the app [create-orthacms-app:I-05]', () => {
        const files = generatedFiles();

        expect(files.length).toBeGreaterThan(30);
        for (const file of files) {
            expect(rendered(file)).not.toMatch(/__[A-Z_]+__/);
            expect(rendered(file)).not.toMatch(/orthacms:(if|ifnot|end)/);
        }
    });

    it('renames every .tmpl file, at any depth [create-orthacms-app:I-22]', () => {
        expect(
    ...
    /**
     * Otherwise `orthacms build` compiles the tests into `dist/server` and ships
     * them — along with whatever fixtures they import.
     */
    it('keeps the unit specs out of the build [create-orthacms-app:I-24]', () => {
     ...
     * has read the Ortha CMS source finds the same shape in their project — and so
     * the CLI's `LAYOUT` constants have one tree to describe.
     *
     * The server contributes **two** projects. `orthacms build` compiles
     ...
     */
    it('lays the four apps out like the monorepo [create-orthacms-app:I-23]', () => {
    ...
    it('gives the server specs a project that knows about jest', () => {
        ...
        // `orthacms build` is the only thing that emits; this one only checks.
     ...
     */
    it('names the Vite config .mts [create-orthacms-app:I-26]', () => {
     ...
     */
    it('points Tailwind at the installed packages [create-orthacms-app:I-25]', () => {
...
describe('every combination', () => {
    ...
    it.each(SELECTIONS)(
        'leaves no placeholder or directive behind — %s [create-orthacms-app:I-05]',
        (_label, ids) => {
            scaffold(...ids);
     ...
     * every `.env` — an operator who decides to turn Ortha CMS AI on should find
     ...
     * `MCP_ENABLED=false` inside the `orthacms:if mcp` block, and the block is
     * right — this is the assertion that keeps the two from being conflated
     * again in either direction.
     */
    it.each(SELECTIONS)(
        'writes the copilot switch always and the MCP switch only when chosen — %s [create-orthacms-app:I-15]',
        (_label, ids) => {
            scaffold(...ids);
            const env = rendered('.env');
     ...
     */
    it('assembles package.json in code rather than through blocks [create-orthacms-app:I-06]', () => {
        const source = readFileSync(
            join(TEMPLATE, 'package.json.tmpl'),
            'utf8'
        );

        expect(source).not.toMatch(/orthacms:(if|ifnot|end)/);
        expect(source).not.toContain('@orthacms/');
    });

    it.each(SELECTIONS)(
        'still writes an installable manifest — %s [create-orthacms-app:I-06]',
        (_label, ids) => {
            scaffold(...ids);
     ...
     * `npx create-orthacms-app@0.3.0` still generates a 0.3.0 app. A version
     ...
     */
    it('writes no @orthacms version down anywhere [create-orthacms-app:I-04]', () => {
        const sources = [
            [
                'src/lib/features.ts',
    ...
    it('lists no versions in features.ts at all [create-orthacms-app:I-04]', () => {
        expect(
            readFileSync(join(__dirname, 'features.ts'), 'utf8')
        ).not.toMatch(/\d+\.\d+\.\d+/);
    });
     ...
     * kind of mistake a stray `orthacms:end` makes in a file nobody reads.
     */
    it.each(
        MEDIA_PROVIDERS.map(
        ...
        'installs and constructs exactly one storage adapter — %s [create-orthacms-app:I-11]',
        (id, pkg) => {
            scaffold(id, 'rest');
 ...
 * `orthacms:if` block carries its own imports and only one survives — the source
...
describe("the generated app's config modules", () => {
    ...
    it.each(configFiles)('%s names no process.env of its own', (file) => {
        // A subscript, not the bare identifier: `orthacms.config.ts`'s header
