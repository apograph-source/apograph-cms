...
gap that stopped a Microsoft-shop deployment running Apograph at all.
...
`npx nx test @apograph/media-provider-azure` runs the shared contract
...
- `npx nx test @apograph/media-provider-azure` /
  `npx nx typecheck @apograph/media-provider-azure` /
  `npx nx lint @apograph/media-provider-azure`
