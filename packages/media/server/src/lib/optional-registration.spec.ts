...
import { WorkspacePurgeRegistry } from '@ortha/workspaces-server';
import { STORAGE_PROVIDER } from '@ortha/media-domain';
 ...
 * shared tool registry (`@ortha/tools-server`, present only when the copilot
 * or MCP is mounted) and the workspace purge registry
 * (`@ortha/workspaces-server`). Both injections are `@Optional()`, so a
