...
or `infrastructure/`.** Only `@apograph/database`'s framework-free
`createDomainEvent`/`DomainEvent`, `@apograph/media-domain`'s storage port, and
node built-ins.

That `@apograph/database` edge is why the aggregates stayed here when the port
...
- **`StorageProvider`** lives in **`@apograph/media-domain`**, not here: a
  ...
  (`@apograph/media-provider-local`, `-s3`) and are constructed at the host's
...
- **Writing a provider** is one factory function plus one
  `describeStorageProvider` call from `@apograph/media-provider-testkit`, which
...
the port, media binds it (hence the `@apograph/content-server` dependency; no
...
([`@apograph/tools-server`](../../tools/server/AGENTS.md)). Each binder injects
...
`npx nx run @apograph/media-server:db:generate --name=<change>` then
...
- `npx nx typecheck @apograph/media-server` / `npx nx lint @apograph/media-server`
- `npx nx test @apograph/media-server` — the package's own jest suite. Besides
