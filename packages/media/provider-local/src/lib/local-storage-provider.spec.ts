...
describe('createLocalStorageProvider', () => {
    ...
    beforeEach(async () => {
        root = await mkdtemp(join(tmpdir(), 'orthacms-local-storage-'));
    ...
    describe('get', () => {
        ...
        it('rejects a key that traverses outside the root instead of reading the file', async () => {
            const outside = await mkdtemp(join(tmpdir(), 'orthacms-outside-'));
    ...
    describe('containment', () => {
        ...
        it('works when rootDir is itself a symlink to another volume', async () => {
            const volume = await mkdtemp(join(tmpdir(), 'orthacms-volume-'));
