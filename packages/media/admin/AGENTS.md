...
to `@ortha/media-server`** over `apiClient`.
...
`@ortha/content-admin` dependency; the tab appears only when the open type
...
- **`MediaFieldSection`** — one field as a **titled card**, deliberately the same
  ...
  `Tooltip`, the **shared** `ChangedBadge` imported from `@ortha/content-admin`
...
by `@ortha/wysiwyg-admin` (hence that dependency; it runs media → wysiwyg,
...
`@ortha/insights-admin`'s `INSIGHTS_WIDGET_SLOT` — media owns `media_asset`,
...
messages namespaced `media.<area>.<key>`; UI from `@ortha/design-system` only.
...
- `npm exec nx typecheck @ortha/media-admin`
- `npm exec nx lint @ortha/media-admin`
- `npm exec nx test @ortha/media-admin`
