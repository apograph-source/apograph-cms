...
The **storage port** — the interface every `@apograph/media-provider-*` speaks,
...
is a *value*, it came from `@apograph/media-server`, and that package's root
barrel re-exports `MediaModule`. So `require('@apograph/media-provider-local')`
loaded `@nestjs/common`, and `@apograph/media-server` sat in every adapter's
runtime `dependencies`, which made `npm i @apograph/media-provider-s3` install
...
`@apograph/media-server`'s `domain/` — framework-free in their own right, but
they import `@apograph/database` for the `DomainEvent` contract, and no adapter
has ever needed them. `@apograph/identity-domain` is scoped the same way: the
...
It is the DI token the composition root binds, and `@apograph/transfer-server`
injects it too. Lockstep versioning is what makes that safe — every package
asks for the same version, so npm keeps one copy.

## Commands

- `npx nx test @apograph/media-domain`
- `npx nx typecheck @apograph/media-domain`
