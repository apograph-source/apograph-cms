...
async function orthacms(...argv: string[]): Promise<void> {
    process.argv = ['node', '/n_m/.bin/orthacms', ...argv];
    jest.resetModules();
...
describe('the .env is read once, before anything is dispatched', () => {
     ...
     * command, `orthacms migrate` would fail on a `DATABASE_URL` sitting right
...
describe('dispatch', () => {
    ...
    it.each(['dev', 'build'])(
        ...
        async (command) => {
            await orthacms(command, '--server', '--admin');
    ...
    it('passes the migration name through', async () => {
        await orthacms('generate', '--name', 'add_posts');
    ...
    it('turns the studio port into a number, leaving an unasked-for one undefined', async () => {
        await orthacms('studio', '--host=0.0.0.0', '--port=5000');
        expect(studioCommand).toHaveBeenCalledWith(ROOT, {
            host: '0.0.0.0',
            port: 5000
        });

        await orthacms('studio');
    ...
    it('reports a --port that is not a number, and runs nothing', async () => {
        await orthacms('studio', '--port=abc');
