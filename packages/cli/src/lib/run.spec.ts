...
function tempApp(packages: Record<string, unknown>): string {
    ...
    const root = realpathSync(mkdtempSync(join(tmpdir(), 'orthacms-cli-run-')));
    roots.push(root);
    writeFileSync(
...
describe('resolving the app’s binaries', () => {
     ...
     *
     * From the *app*: `orthacms` is installed into the app it builds, so the
...
describe('spawnNode', () => {
    ...
    it('hands the child this process’s environment [cli:I-15]', () => {
        fakeChild();
        process.env.ORTHACMS_SPEC_INHERITED = 'yes';
        ...
        try {
            ...
            delete process.env.ORTHACMS_SPEC_INHERITED;
        }
        ...
        expect(options.env.ORTHACMS_SPEC_INHERITED).toBe('yes');
...
describe('superviseUntilExit', () => {
     ...
     * and a `node --watch` still holding the API port — so the *next* `orthacms
