/**
 * Argv parsing for the `orthacms` binary.
 *
...
export const USAGE = `orthacms — the Ortha CMS command line
 ...
 * nothing: `orthacms build --server --admin` used to compile nothing, build
...
export function halves(argv: readonly string[], command: string): Halves {
    ...
    if (serverOnly && adminOnly) {
        throw new Error(
            `\`orthacms ${command} --server --admin\` asks for neither half — ` +
 ...
 * command. `orthacms --help` puts `--help` in the command position, and a check
 ...
 *
 * Checked before {@link wantsHelp}, so `orthacms --version --help` answers with
