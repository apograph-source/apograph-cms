 ...
 */
const scratch = mkdtempSync(join(tmpdir(), 'orthacms-cli-env-'));
...
interface ChildRun {
    /** The child's own `ORTHACMS_SPEC_*` variables after `loadEnv` returned. */
...
function loadEnvIn(
    ...
    const script = [
        ...
        `    Object.entries(process.env).filter(([k]) => k.startsWith('ORTHACMS_SPEC_'))`,
        `)));`
    ].join('\n');
...
describe('loadEnv', () => {
    ...
    it('leaves an already-exported variable alone [cli:I-14]', () => {
        const root = app(
            'ORTHACMS_SPEC_EXPORTED=from-the-file\nORTHACMS_SPEC_FROM_FILE=only-in-the-file\n'
        );

        const { env, status } = loadEnvIn(root, {
            ORTHACMS_SPEC_EXPORTED: 'from-the-shell'
        });

        expect(status).toBe(0);
        expect(env.ORTHACMS_SPEC_EXPORTED).toBe('from-the-shell');
        // The same run did read the file — so the line above is about
        // precedence, not about the file having been skipped wholesale.
        expect(env.ORTHACMS_SPEC_FROM_FILE).toBe('only-in-the-file');
    });

    it('reads the .env beside the project root it was given [cli:I-14]', () => {
        const root = app('ORTHACMS_SPEC_FROM_FILE=beside-the-root\n');
        ...
        expect(loadEnvIn(root).env.ORTHACMS_SPEC_FROM_FILE).toBe(
            'beside-the-root'
        );
    });
