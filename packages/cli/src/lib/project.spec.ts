...
function tempApp(): string {
    const root = mkdtempSync(join(tmpdir(), 'orthacms-cli-project-'));
    roots.push(root);
    writeFileSync(
...
describe('findProjectRoot', () => {
    /**
     * People run `orthacms migrate` from wherever they happen to be — most often
    ...
    it('says to run inside an app when the walk reaches the filesystem root [cli:I-24]', () => {
        ...
        const orphan = mkdtempSync(join(tmpdir(), 'orthacms-cli-orphan-'));
        roots.push(orphan);

        expect(() => findProjectRoot(orphan)).toThrow(
            /No package\.json in .* run this inside an Ortha CMS app/s
        );
    });
});
...
describe('loadHost', () => {
    const CONFIG_SOURCE = `
        exports.default = {
            database: { url: 'postgresql://orthacms:secret@db:5432/app' },
    ...
    it('reads the compiled config itself, not an ESM wrapper around it [cli:I-23]', () => {
        ...
        expect(config.database?.url).toBe(
            'postgresql://orthacms:secret@db:5432/app'
        );
    });
    ...
    it('builds the plugin list from that same config object [cli:I-23]', () => {
        ...
        expect(plugins).toEqual([
            {
                name: 'database',
                url: 'postgresql://orthacms:secret@db:5432/app'
            }
        ]);
    });
    ...
    it('names the missing default export when orthacms.config.ts has none', () => {
        const root = tempApp();
        writeCompiledHost(root, {
...
describe('requireDatabaseUrl', () => {
    ...
    it('returns a configured url unchanged [cli:I-03]', () => {
        ...
        ).toBe('postgresql://orthacms:secret@db:5432/app');
    });
});
