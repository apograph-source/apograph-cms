 ...
 * (`@ortha/content-domain` declares no dependencies) is pinned in that
 ...
 * engine it was extracted from. The `@ortha/nx` layer-boundary lint is not
 ...
 * *does* import `@ortha/content-domain` (the status state machine and the
 * publish gate — the whole point of a shared kernel) and `@ortha/database`
...
describe('entries/domain layer imports', () => {
    ...
    it('does depend on the shared kernel — framework-free, not dependency-free', () => {
        ...
        const kernelUsers = FILES.filter((path) =>
            specifiersOf(path).includes('@ortha/content-domain')
        );
