...
export const ENTRY_TAB = {
     ...
     * `@apograph/segments-admin`. A slug the router knows, declared here rather
     ...
     * `@apograph/activity-admin`. Declared here for the same reason as Access:
 ...
 * `CONTENT_FIELD_TYPE` (`@apograph/content-server`). Kept as a local constant
 ...
 * server's `BULK_VERDICT` (`@apograph/content-server`). Drives the icon/label
