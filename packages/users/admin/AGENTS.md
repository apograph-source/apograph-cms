...
`@apograph/activity-admin`'s `useActivityLog`, pinned to `subjectId`),
...
- Data flows through the gateway seam: `application/` hooks call
  ...
  `@apograph/utils-admin` (no local copies).
...
(`npx nx test @apograph/users-admin`) hold only what a browser can't reach: the
...
- `npx nx typecheck @apograph/users-admin` / `npx nx lint @apograph/users-admin`
- `npx nx test @apograph/users-admin` — the unit specs above
