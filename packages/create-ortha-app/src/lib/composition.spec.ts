...
function importedFrom(source: string): Map<string, string> {
    ...
    for (const [, specifiers, pkg] of source.matchAll(
        /^import\s+\{([\s\S]*?)\}\s+from\s+'(@ortha\/[^']+)'/gm
    )) {
...
describe('the mail plugin', () => {
    it('is composed when a backend was chosen [create-ortha-app:I-35]', () => {
    ...
    it('leaves no husk in an app that sends nothing [create-ortha-app:I-35]', () => {
