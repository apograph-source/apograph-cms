 ...
 *
 * `apograph.config.ts` reads `process.env` at import time, so this has to be a
...
process.env['APOGRAPH_ROOT_ADMIN_EMAIL'] ||= 'e2e@example.com';
process.env['APOGRAPH_ROOT_ADMIN_PASSWORD'] ||= 'e2e-password-not-a-secret';
