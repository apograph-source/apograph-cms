...
describe('the templates directory', () => {
     ...
     * Nx is the one that fails hardest: left visible, `@apograph/nx` infers a
    ...
    ])('is excluded from %s [create-apograph-app:I-31]', (_name, read) => {
        expect(read()).toMatch(/packages\/create-apograph-app\/templates/);
    });

    it('is excluded from the TypeScript build [create-apograph-app:I-31]', () => {
...
describe('the package itself', () => {
     ...
     */
    it('installs nothing to run [create-apograph-app:I-32]', () => {
