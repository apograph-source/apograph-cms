 ...
 * expected list carries `apograph:if` blocks of its own, so it has to stay right
...
describe('the generated app’s server composition', () => {
    it.each(
        ...
        'ships a plugins.spec.ts that expects what plugins.ts registers — %s [create-apograph-app:I-29]',
        (_label, ids) => {
     ...
     */
    it('opens the database first and migrates identity before workspaces [create-apograph-app:I-27]', () => {
    ...
    it('registers MCP last, where present [create-apograph-app:I-27]', () => {
...
describe('the generated app’s admin composition', () => {
    it('ships a plugins.spec.ts that expects what plugins.ts registers [create-apograph-app:I-29]', () => {
     ...
     */
    it('lets only the shell supply a layout, and puts identity first [create-apograph-app:I-28]', () => {
 ...
 * design — an unpicked media adapter or an `apograph:if`-guarded protocol is
...
describe('every core plugin package is actually mounted', () => {
    ...
    it('recognises the core packages that define plugins', () => {
        ...
        expect(pluginPackages).toContain('@apograph/webhooks-server');
        expect(pluginPackages).toContain('@apograph/webhooks-admin');
        expect(pluginPackages).not.toContain('@apograph/utils-admin');
        expect(pluginPackages).not.toContain('@apograph/webhooks-domain');
    });

    // covers: create-apograph-app:I-33
 ...
 * `apograph:if` block must be read by something that survives the same block —
...
describe('the generated .env has no key nothing reads', () => {
    ...
    // covers: create-apograph-app:I-34
    it.each([
