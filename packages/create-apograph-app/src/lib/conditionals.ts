/**
 * `apograph:if` blocks — how one template serves every feature combination.
 *
...
const IF = new RegExp(
    String.raw`(^|\s)apograph:if\s+([a-z0-9-]+)${CLOSER}\s*$`
);
const IFNOT = new RegExp(
    String.raw`(^|\s)apograph:ifnot\s+([a-z0-9-]+)${CLOSER}\s*$`
);
const END = new RegExp(String.raw`(^|\s)apograph:end${CLOSER}\s*$`);
...
/**
 * Applies every `apograph:if` block in `contents` against the enabled `flags`.
 *
 * @throws when a block is left open, which would otherwise silently swallow the
 * rest of the file — a missing `apograph:end` in `plugins.ts` deletes every plugin
...
export function applyConditionals(
    ...
    for (const line of lines) {
        ...
        if (directive.kind === 'end') {
            if (stack.length === 0) {
                throw new Error(
                    `${file}: an \`apograph:end\` with no matching \`apograph:if\`.`
                );
            }
            stack.pop();
            continue;
    ...
    if (stack.length > 0) {
        throw new Error(
            `${file}: ${stack.length} unclosed \`apograph:if\` block(s) — every ` +
                `one needs an \`apograph:end\`, or the rest of the file is dropped.`
        );
    }
