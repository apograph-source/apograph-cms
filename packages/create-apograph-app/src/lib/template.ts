...
export interface TemplateValues {
    ...
    /** The `@apograph/*` version every dependency is pinned to. */
    apographVersion: string;
...
export function render(contents: string, values: TemplateValues): string {
    const replacements: Record<string, string> = {
        ...
        __APOGRAPH_VERSION__: values.apographVersion
    };
 ...
 * with the non-Apograph dependencies and an empty `@apograph` set, and the chosen
 * packages are merged in and re-sorted here. Every `@apograph/*` range is the
...
export function renderManifest(
    ...
    const pin = (names: readonly string[]): Record<string, string> =>
        Object.fromEntries(names.map((name) => [name, values.apographVersion]));
...
export function renderTemplate(
    ...
    for (const file of filesIn(templateDir)) {
        ...
        // the whole of `config/mcp.ts` in `apograph:if mcp` and an app generated
