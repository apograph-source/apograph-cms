...
beforeEach(() => {
    target = mkdtempSync(join(tmpdir(), 'create-apograph-'));
});
...
describe('the scaffolded app, whatever the features', () => {
    ...
    it('pins every @apograph dependency to this scaffolder’s version [create-apograph-app:I-03]', () => {
        const { dependencies, devDependencies } = manifest();
        const apograph = Object.entries({
            ...dependencies,
            ...devDependencies
        }).filter(([name]) => name.startsWith('@apograph/'));

        expect(apograph.length).toBeGreaterThan(0);
        for (const [, range] of apograph) expect(range).toBe('9.9.9');
    });
    ...
    it('pins them exactly, with no caret', () => {
        for (const [name, range] of Object.entries(manifest().dependencies)) {
            if (!name.startsWith('@apograph/')) continue;
            expect(range.startsWith('^')).toBe(false);
        }
    });

    it('keeps the non-Apograph dependencies the template declares', () => {
     ...
     */
    it('restores the dotfiles npm will not publish [create-apograph-app:I-22]', () => {
    ...
    it('renames the .tmpl files onto their real names [create-apograph-app:I-22]', () => {
    ...
    it('scaffolds no signing secrets, because identity has none to sign with [create-apograph-app:I-17]', () => {
     ...
     */
    it('leaves no placeholder or directive anywhere in the app [create-apograph-app:I-05]', () => {
        const files = generatedFiles();

        expect(files.length).toBeGreaterThan(30);
        for (const file of files) {
            expect(rendered(file)).not.toMatch(/__[A-Z_]+__/);
            expect(rendered(file)).not.toMatch(/apograph:(if|ifnot|end)/);
        }
    });

    it('renames every .tmpl file, at any depth [create-apograph-app:I-22]', () => {
        expect(
    ...
    /**
     * Otherwise `apograph build` compiles the tests into `dist/server` and ships
     * them — along with whatever fixtures they import.
     */
    it('keeps the unit specs out of the build [create-apograph-app:I-24]', () => {
     ...
     * has read the Apograph source finds the same shape in their project — and so
     * the CLI's `LAYOUT` constants have one tree to describe.
     *
     * The server contributes **two** projects. `apograph build` compiles
     ...
     */
    it('lays the four apps out like the monorepo [create-apograph-app:I-23]', () => {
    ...
    it('gives the server specs a project that knows about jest', () => {
        ...
        // `apograph build` is the only thing that emits; this one only checks.
     ...
     */
    it('names the Vite config .mts [create-apograph-app:I-26]', () => {
     ...
     * like `@apograph/*\/dist/**\/*.js` matches zero files and the build is
     ...
     */
    it('points Tailwind at the installed packages [create-apograph-app:I-25]', () => {
        const styles = rendered('apps/admin/src/styles.css');
        expect(styles).toContain('@source "../../../node_modules/@apograph";');
...
describe('with nothing optional chosen', () => {
    ...
    it('installs no model backend at all', () => {
        ...
        expect(names).not.toContain('@apograph/copilot-provider-anthropic');
        expect(names).not.toContain('@apograph/copilot-provider-openai');
        expect(names).not.toContain('@apograph/copilot-provider-fake');
...
describe('every combination', () => {
    ...
    it.each(SELECTIONS)(
        'leaves no placeholder or directive behind — %s [create-apograph-app:I-05]',
        (_label, ids) => {
            scaffold(...ids);
     ...
     * every `.env` — an operator who decides to turn Apograph AI on should find
     ...
     * `MCP_ENABLED=false` inside the `apograph:if mcp` block, and the block is
     * right — this is the assertion that keeps the two from being conflated
     * again in either direction.
     */
    it.each(SELECTIONS)(
        'writes the copilot switch always and the MCP switch only when chosen — %s [create-apograph-app:I-15]',
        (_label, ids) => {
            scaffold(...ids);
            const env = rendered('.env');
     ...
     * `@apograph` package at all.
     */
    it('assembles package.json in code rather than through blocks [create-apograph-app:I-06]', () => {
        const source = readFileSync(
            join(TEMPLATE, 'package.json.tmpl'),
            'utf8'
        );

        expect(source).not.toMatch(/apograph:(if|ifnot|end)/);
        expect(source).not.toContain('@apograph/');
    });

    it.each(SELECTIONS)(
        'still writes an installable manifest — %s [create-apograph-app:I-06]',
        (_label, ids) => {
            scaffold(...ids);
            ...
            expect(Object.keys(parsed.dependencies)).toEqual(
                expect.arrayContaining(['@apograph/content-server', 'react'])
            );
        }
    );
     ...
     * `npx create-apograph-app@0.3.0` still generates a 0.3.0 app. A version
     * written down beside an `@apograph` name anywhere in here is a pin that
     * will be wrong by the next release and correct-looking forever.
     */
    it('writes no @apograph version down anywhere [create-apograph-app:I-04]', () => {
        const sources = [
            [
                'src/lib/features.ts',
        ...
        for (const [name, source] of sources) {
            const pinned = source
                ...
                .filter(
                    (line) =>
                        line.includes('@apograph') && /\d+\.\d+\.\d+/.test(line)
                );
    ...
    it('lists no versions in features.ts at all [create-apograph-app:I-04]', () => {
        expect(
            readFileSync(join(__dirname, 'features.ts'), 'utf8')
        ).not.toMatch(/\d+\.\d+\.\d+/);
    });
     ...
     * kind of mistake a stray `apograph:end` makes in a file nobody reads.
     */
    it.each(
        MEDIA_PROVIDERS.map(
        ...
        'installs and constructs exactly one storage adapter — %s [create-apograph-app:I-11]',
        (id, pkg) => {
            scaffold(id, 'rest');

            const adapters = Object.keys(manifest().dependencies).filter(
                (name) => name.startsWith('@apograph/media-provider-')
            );
...
describe('a file the template ships as data', () => {
     ...
     */
    it('is copied byte for byte, with no substitutions [create-apograph-app:I-30]', () => {
        const source = mkdtempSync(join(tmpdir(), 'create-apograph-src-'));
 ...
 * `apograph:if` block carries its own imports and only one survives — the source
...
describe("the generated app's config modules", () => {
    ...
    it.each(configFiles)('%s names no process.env of its own', (file) => {
        // A subscript, not the bare identifier: `apograph.config.ts`'s header
