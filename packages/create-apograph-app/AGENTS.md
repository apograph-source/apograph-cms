...
The scaffolder. `npx create-apograph-app my-cms` writes an Apograph CMS app that
consumes every package from npm.

## Package

- Name: **`create-apograph-app`** — unscoped, so `npx create-apograph-app` and
  `npm create apograph-app` both work. It is the one package in this workspace
  outside the `@apograph` scope, which is why `nx.json`'s `release.projects`
  names it explicitly alongside the `@apograph/*` glob.
- Binary: `create-apograph-app`
- Released **in lockstep** with everything else.

## The version mechanism

The scaffolder stamps **its own version** into every `@apograph/*` dependency of
the generated app (`__APOGRAPH_VERSION__`). Since the release is lockstep, its
version _is_ the matching set — so `npx create-apograph-app@0.4.0` generates a
...
registry is also what makes `create-apograph-app@<old>` reproducible.

## Keeping the template current

Two upkeep rules, both enforced by tests rather than by memory.

**Releasing does not require a template edit.** Every `@apograph/*` dependency
is written as `__APOGRAPH_VERSION__` and stamped with the scaffolder's own version
...
**Adding a package does require one.** Every published `@apograph/*` package
...
`@apograph/*` package it could import, the extension points included — the
...
absent from `plugins.ts` and its config block from `apograph.config.ts` — which is
...
| Where                         | Why                                                                                                                                                                                                                                                                  |
| ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `.nxignore`                   | Nx inference walks every directory. Left visible, `@apograph/nx` infers a `db:migrate` target onto `templates/default/src/server` — a directory with no project name — and **the whole project graph fails to build**, taking every `nx` command in the repo with it |
...
└── apps/
    ├── server/           apograph.config.ts, src/{main,plugins}.ts, jest.config.js
...
Apograph source finds the same shape in their project — and `LAYOUT` in
`@apograph/cli` has one tree to describe rather than two.
...
internal deps per release, so a split risks two `@apograph/design-system`
...
makes the compiled paths predictable: `apps/server/apograph.config.ts` →
`dist/server/apograph.config.js`, `apps/server/src/main.ts` →
...
and `apograph start` reports a missing entry point rather than a moved one.
...
2. **`@source "../../../node_modules/@apograph"`** in `styles.css`. Tailwind
   ...
   ignore rules, so `@apograph/*/dist/**/*.js` matches nothing and the emitted
...
One template serves every feature combination through `apograph:if` markers
...
`apograph:if`, `apograph:ifnot` and `apograph:end`, on any comment syntax, so the same
...
feature is off — and `apograph:if` cannot drop it, because two features may need
...
3. `templates/default/apps/server/config/<plugin>.ts` plus a line in
   `apograph.config.ts` — **only if the plugin has settings an operator sets**.
...
order. They are **feature-aware**: the expected server list carries `apograph:if`
...
`applyPluginMigrations` — the same function `apograph migrate` calls, so the schema
...
- **`jest.setup.js` supplies placeholder secrets.** The specs import
  `apograph.config.ts`, which deliberately refuses to load without `DATABASE_URL`.
...
- **`tsconfig.server.json` excludes `**/\*.spec.ts`**, or `apograph build`compiles
...
`npx nx test create-apograph-app` — the package-coverage guard, the conditional
