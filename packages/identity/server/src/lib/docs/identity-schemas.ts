...
const SSO_BACKCHANNEL_LOGOUT_SCHEMA: OpenApiSchema = {
    type: 'object',
    description:
        'How many Apograph sessions the notification ended. `0` is a normal answer: the endpoint is idempotent, because a provider retrying a notification is expected.',
    required: ['revoked'],
...
const API_TOKEN_SCHEMA: OpenApiSchema = {
    ...
    properties: {
        ...
        lookupPrefix: {
            type: 'string',
            description:
                "The token's non-secret leading characters (e.g. `apograph_0TYI4Q`), so a person can tell two tokens apart in a list."
        },
