...
The admin counterpart to `@ortha/i18n-server` — content localization in the
Content Library UI. It contributes **no routes, no layout, no nav item**: eight
of its nine contributions fill `@ortha/content-admin`'s extension slots, and
...
It is the reason this package depends on `@ortha/insights-admin` — the same
...
- `npx nx typecheck @ortha/i18n-admin` / `npx nx lint @ortha/i18n-admin`
- `npx nx test @ortha/i18n-admin` — the unit tests (vitest, jsdom)
