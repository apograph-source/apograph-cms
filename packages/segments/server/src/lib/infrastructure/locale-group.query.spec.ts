...
describe('the locale group is answered without i18n', () => {
     ...
     * have installed. If it did, an installation without `@ortha/i18n-server`
     * would fail to resolve the group — and the failure would land on the write
     * that decides who may read published content.
     */
    it('needs no dependency on @ortha/i18n-server [segments:I-15]', () => {
        ...
        expect(Object.keys(declared)).not.toContain('@ortha/i18n-server');
        expect(
            readFileSync(join(__dirname, 'locale-group.query.ts'), 'utf-8')
        ).not.toContain("from '@ortha/i18n-server'");
    });
});
