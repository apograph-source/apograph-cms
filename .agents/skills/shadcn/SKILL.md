...
allowed-tools: Bash(npx shadcn@latest *), Bash(pnpm dlx shadcn@latest *), Bash(bunx --bun shadcn@latest *), Read, Edit, Write, Glob, Grep
---
...
## Project Conventions (apograph-cms monorepo)

**This is an Nx monorepo. `components.json` lives in `packages/design-system`, not the repo root.** Running any shadcn CLI command from the root errors with `monorepo_root`. Always target the workspace with `-c packages/design-system`:

```bash
npx shadcn@latest add button -c packages/design-system
npx shadcn@latest info --json -c packages/design-system
```

After adding or updating a component, **two repo-specific steps are mandatory** (both require the Read/Edit tools):

1. **Rewrite `@/` imports to relative paths.** The library is consumed from source by `apps/admin` via Vite, and Vite's dev server does not resolve the library-local `@/*` tsconfig alias (it fails with `Failed to resolve import "@/lib/utils"`). The CLI emits `@/`-prefixed imports — rewrite every one in the added file(s) to a relative path:
   - `@/lib/utils` → `../../utils` (from a file in `src/lib/components/ui/`)
   - `@/lib/components/ui/<x>` → `./<x>` (sibling UI component)
   - `@/lib/hooks/<x>` → `../../hooks/<x>`
2. **Export from the barrel.** Add the component's exports (and any `*Props` types) to `packages/design-system/src/index.ts` so consumers can `import { X } from '@apograph/design-system'`.
...
- **Use `truncate` shapographnd.** Not `overflow-hidden text-ellipsis whitespace-nowrap`.
