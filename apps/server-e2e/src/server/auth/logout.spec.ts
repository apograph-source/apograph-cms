...
/** Grab the raw Set-Cookie string for `orthacms_session` (with attributes). */
function rawSetCookie(res: Response): string | undefined {
    const setCookie = res.headers['set-cookie'] as unknown as
        | string[]
        | undefined;
    return setCookie?.find((c) => c.startsWith('orthacms_session='));
}
...
describe('POST /api/auth/logout', () => {
    ...
    it('returns 201 { ok: true } and clears the cookie', async () => {
        ...
        expect(cleared).toMatch(/^orthacms_session=;/);
