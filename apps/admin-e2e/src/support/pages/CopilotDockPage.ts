...
export class CopilotDockPage extends BasePage {
     ...
     * identical "Ortha CMS AI"s. That means the name is no longer a constant to
     * match on, and the grip is the one control every panel has and nothing
     * else does.
     */
    windows(): Locator {
        return this.page.getByRole('dialog').filter({
            has: this.page.getByRole('button', { name: 'Move Ortha CMS AI' })
        });
    }
    ...
    moveHandle(index = 0): Locator {
        return this.headerButton('Move Ortha CMS AI', index);
    }

    /** The window's visible title — the thread's name, or "Ortha CMS AI". */
    ...
    async storedFrame(slot = 0): Promise<unknown> {
        ...
        }, `orthacms.copilot.panel-frame.${slot}`);
    }
}
