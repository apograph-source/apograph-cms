 ...
 */
test.describe('Apograph AI dock', () => {
    ...
    test('is the entry point, and opens a window focused on the composer [shell:I-13]', async ({
        ...
        await expect(copilotDockPage.dock).toContainText('Apograph AI');
 ...
 */
test.describe('Apograph AI dock — the attached page', () => {
 ...
 */
test.describe('Apograph AI dock — regressions', () => {
    test.beforeEach(async ({ page, contentLibraryPage }) => {
        await mockSignedIn(page);
        await mockWorkspaces(page);
        await mockContentSchema(page);
    ...
    test('the history dropdown will not open a thread a second window already holds', async ({
        ...
        // transcript.
        await expect(copilotDockPage.panelTitle(0)).toHaveText(THREAD);
        await expect(copilotDockPage.panelTitle(1)).toHaveText('Apograph AI');
    ...
    test('the start button announces the label it shows, and the shortcut its platform accepts', async ({
        ...
        // 2.5.3 Label in Name: the visible text is "Apograph AI" and the
        // accessible name was the constant "New chat", so the two had nothing
        // in common — "click Apograph AI" did not work by voice.
        await expect(start).toHaveText(/Apograph AI/);
        await expect(start).toHaveAccessibleName(/Apograph AI/);
    ...
    test('the dock is a group, and two windows have two names', async ({
        ...
        // Both windows used to answer to the accessible name "Apograph AI", so a
...
test.describe('Apograph AI dock accessibility (axe, WCAG 2.1 A/AA)', () => {
    test.beforeEach(async ({ page, contentLibraryPage }) => {
        await mockSignedIn(page);
        await mockWorkspaces(page);
        await mockContentSchema(page);
