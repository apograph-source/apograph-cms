...
/** Reloads `apograph.config` with `env` applied over the ambient environment. */
function loadConfig(env: Record<string, string | undefined>): ApographConfig {
    ...
    try {
        let loaded: ApographConfig | undefined;
        jest.isolateModules(() => {
            loaded = require('../apograph.config').default as ApographConfig;
        });
        return loaded as ApographConfig;
    } finally {
        process.env = saved;
    }
}
 ...
 * fall back to. The readers in `@apograph/utils-server` decide "empty means not
...
describe('a variable that is set but empty', () => {
    ...
    it('refuses a root administrator password made of spaces', () => {
        ...
        expect(
            loadConfig({
                APOGRAPH_ROOT_ADMIN_EMAIL: 'admin@example.test',
                APOGRAPH_ROOT_ADMIN_PASSWORD: '   '
 ...
 * prose in `AGENTS.md` and a sentence in `apograph.config.ts`'s header. The
