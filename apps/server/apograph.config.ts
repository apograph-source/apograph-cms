/**
 * Typed application configuration for the Apograph CMS server.
 *
 * `config/` is the single place that reads the environment, and it reads it
 * **only** through the readers in `@apograph/utils-server` — no module in the
 * folder touches `process.env` itself, which `src/apograph.config.spec.ts`
 ...
 * folder: `@apograph/cli` looks for exactly `dist/server/apograph.config.js`
 * (`LAYOUT.compiledConfig`), `@apograph/nx` infers the migration targets onto
 * the project that has an `apograph.config.ts`, and the types below are imported
 * from here by `src/plugins.ts` and by `apps/server-e2e`.
 *
 * *How* a value is parsed is not decided in this folder at all: the readers
 * come from `@apograph/utils-server`, shared with the scaffolder's template so
...
import { identityConfig, type ApographIdentityConfig } from './config/identity';
import { i18nConfig } from './config/i18n';
import { mediaConfig, type ApographMediaConfig } from './config/media';
import { copilotConfig, type ApographCopilotConfig } from './config/copilot';
...
export interface ApographConfig {
    ...
    /** Database connection settings. */
    database: ApographDatabaseConfig;
    ...
    plugins: {
        ...
        identity: ApographIdentityConfig;
        ...
        media: ApographMediaConfig;
        /** Copilot plugin settings — kill switch + model providers. */
        copilot: ApographCopilotConfig;
