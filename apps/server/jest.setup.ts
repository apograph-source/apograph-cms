/**
 * Supplies the one deploy value `orthacms.config.ts` refuses to default.
 *
 * The specs here assert the **real** composition — `orthacms.config.ts` and
...
process.env['DATABASE_URL'] ??=
    'postgres://spec:spec@127.0.0.1:5432/orthacms_spec';
