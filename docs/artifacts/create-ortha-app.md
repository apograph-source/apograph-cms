...
The scaffolder is **the single entry point into the product for a new user**. `npx create-ortha-app my-cms` asks a few questions and leaves an application on disk that builds, migrates and runs without a single edit. Technically it is a generator with one template; as a product it is a shop window: it decides how Ortha is seen the first time, and what a person gets by default, choosing nothing.
...
Gets the same four-application layout as the Ortha monorepo itself: `server`, `admin`, `server-e2e`, `admin-e2e`. Everything they read in the CMS's source sits in the same place in theirs.
...
Both packages in the manifest; a derived **`mail`** group flag (the second after `sso`); `config/mail.ts`; a `plugins.mail?: AppMailConfig` field and a `mail: mailConfig()` line in `ortha.config.ts`; a `mailPlugin()` helper and a `...mailPlugin(config)` entry **before** `UsersPlugin()` in `plugins.ts`; the `MAIL_*` and `SMTP_*` blocks in `.env`; an "Outgoing mail" section in the README.
...
Tailwind excludes `node_modules` from content auto-detection. Without the line `@source "../../../node_modules/@ortha";` the whole admin UI renders **unstyled**, and nothing fails.
...
5. **The summary frame.** The name, the database, the storage, the protocols, the copilot, sign-in, and the number of `@ortha` packages. An empty multi-selection is printed as a muted `none` / `no backend` — so the absence of a choice is shown rather than hidden.
...
`renderManifest()` **rebuilds** the dependency map rather than patching it: the template carries a manifest with its non-Ortha dependencies and an empty `@ortha` set, the chosen packages are merged in, and the whole map is re-sorted.
...
- **I-25** — `styles.css` contains the exact line `@source "../../../node_modules/@ortha";` and **contains no** `@source` with a glob inside a `node_modules` path.
