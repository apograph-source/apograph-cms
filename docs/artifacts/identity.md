...
**Who this is and what they may do — the foundation of access in Ortha CMS**
...
- **It is not email.** Sending belongs to `mail` ([ADR-0018](../adr/0018-mail-provider.md)), and issuing the links belongs to `users`; identity only _redeems_ them. On a deployment that configures no provider the links are still returned to the administrator in the API response and passed on by hand. **Self-service "forgot your password?" is still absent** — it needs a public route with its own rules about enumeration and rate limiting, which is phase 2 of that record — but the reason has changed: there is now somewhere to send it.
...
Issuing the link is the `users` plugin; redeeming it is Identity. In between: the message, when a mail provider is configured — and otherwise the administrator, passing the link to the person by hand.
...
7. **The account is activated and a session is issued straight away.** The response is `{ ok: true }` plus the `orthacms_session` cookie. The invitee is inside the application with no separate sign-in.
...
6. **The cookie is set by the controller.** `orthacms_session`: `httpOnly`, with `secure` and `sameSite` from the configuration, `maxAge` = the session TTL, `path=/`.
...
2. **Starting an attempt.** `GET /api/auth/sso/:provider/start?redirect=…&invite=…`: the core (not the adapter!) generates the `state`, the `nonce` and the PKCE verifier, writes an `sso_auth_requests` row, sets the short-lived `orthacms_sso_request` cookie and issues a `302` to the provider.
...
| ------------------------------------------- | ---------------------------------------------- | ------------------------------------------------------------ | --------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| POST /auth/login                            | `public` Throttler, Origin                     | `{ email, password }`                                        | `200 { ok: true }` + `Set-Cookie: orthacms_session`       | `401` for everything; `403` a foreign Origin; `429` the rate limit; `400` an invalid DTO |
...
| GET /auth/sso/:provider/start               | `public` Throttler                             | `?redirect=`, `?invite=`                                     | `302` to the provider + the `orthacms_sso_request` cookie | `404` an unregistered provider                                                           |
...
- **Correct credentials** → 200, an `orthacms_session` cookie with `HttpOnly`, a `sessions` row in the database, and the cookie's value ≠ `sessions.id`.
