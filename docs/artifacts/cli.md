...
**The `apograph` command — how an application installed from npm is built, run and migrated**

Everything needed to **operate** an Apograph application used to live in `@apograph/nx` — a private package that will never be published. An application installed from npm physically had no way to apply its own migrations. `@apograph/cli` closes that hole and at the same time fixes a rule: **the monorepo and any generated application migrate through one and the same implementation**. Two implementations of "apply migrations in plugin order" are two chances to get the system's most destructive operation wrong.
...
- [10. Boundaries: the CLI and @apograph/nx](#10-boundaries-the-cli-and-apographnx)
...
Apograph is distributed in two ways. Inside the monorepo a developer lives in Nx: `npx nx serve server`, `npx nx run server:db:migrate`. But the end product is an application a person creates with `npx create-apograph-app my-cms`, made almost entirely of `@apograph/*` packages installed from npm. Such an application has no Nx, no executors and no project graph — and needs a way to be built, run and migrated.

That way is `apograph`. Six commands, which a generated application wraps in its own `npm run` scripts, so that the application's owner usually never types the word `apograph` at all:

| What the application's owner does | What they type               | What is invoked         |
| --------------------------------- | ---------------------------- | ----------------------- |
| Develops                          | npm run dev                  | apograph dev               |
| Builds a release                  | npm run build                | apograph build             |
| Runs in production                | npm start                    | apograph start             |
| Creates the database schema       | npm run migrate              | apograph migrate           |
| Sets up their own content types   | npm run generate -- --name=… | apograph generate --name=… |
| Inspects the data                 | npm run studio               | apograph studio            |

### Why this is a separate package

- **Publishability.** `@apograph/nx` is marked `private: true` and will never reach npm — it is tied to `@nx/devkit`, the project graph and the `nx.json` file. The `applyPluginMigrations` function was reachable only through an Nx executor, that is, only from inside the monorepo.
- **One implementation for two worlds.** The `db-migrate`, `db-generate` and `db-studio` executors in `@apograph/nx` are thin adapters over the functions exported here. The same migration loop, the same refusal on an empty `DATABASE_URL`, the same warning on an unsafe Drizzle Studio bind.
...
#### An Apograph developer

Never calls `apograph` directly — they have Nx. But they execute the same logic: three Nx executors import functions from this package.

#### Deployment / CI

Production is exactly `apograph build && apograph start`, one process serving both the API and the admin UI from one origin. Migrations are a separate `apograph migrate` step before the start.
...
- **It is not a server.** `apograph start` runs the already compiled `main.js` with plain `node` and dies with it.
- **It is not a project generator.** Creating an application is `create-apograph-app`. The CLI appears in an application as a `devDependency` afterwards.
...
| ---------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------ |
| src/cli.ts             | The `apograph` binary's entry point: argv parsing, the help text, the command dispatcher, one error catch | Prints a **message** rather than a stack; `process.exit(1)`                                      |
| src/index.ts           | The package's public API — what `@apograph/nx` imports                                                 | 9 exports; they are what makes the implementation shared                                         |
...
| Package                    | What for                                                                                         |
| -------------------------- | ------------------------------------------------------------------------------------------------ |
| @apograph/bootstrap-server | Only for the `ServerPlugin` type — the `migrations: { dir(), table }` descriptor is read from it |
...
### The public API (what `@apograph/nx` imports)
...
>
> In the repository `package.json` declares `"bin": { "apograph": "./src/cli.ts" }` — because every workspace package resolves **from source**. The published tarball fixes it: the `tools/release/pack.mjs` script rewrites the manifest and remaps `bin` exactly as it does `main` (to `./dist/…js`), then checks that the file named in `bin` really was built. The consequence: **inside the monorepo the `apograph` command as such does not work** — Nx plays its role there.

### How the package reaches a user

Between what sits in the repository and what a person installs from npm there is a step without which `apograph` would simply not start. Every workspace package resolves **from source**: their `exports` point at `./src/index.ts` and `tsconfig.base.json` supplies the `@apograph/source` condition. A consumer cannot do that — they have neither the condition nor a compiler at runtime.
...
2. **Staging.** The `pack` target assembles the publishable root under `dist/pack/packages/cli` with a **rewritten** manifest: `main`, `types`, `exports` and `bin` are moved from `./src/*.ts` to `./dist/*.js`, and dependencies on workspace packages (`"*"`) are pinned to the version being released.
   _this is where bin.apograph stops pointing at TypeScript_
...
A practical consequence for debugging follows: `apograph` "as a user sees it" can only be observed on a built tarball, not in a monorepo checkout.
...
```
apograph — the Apograph CMS command line
...
| migrate      | Builds the server (`--server`), reads the compiled config and plugin list, and applies each plugin's migrations in list order                      | —                                                             | `DATABASE_URL`; a reachable database; `dist/server/apograph.config.js` and `dist/server/src/plugins.js` (created by the command itself)        | 0 / 1                               |
...
- **Termination takes everyone with it.** Without that, leaving `dev` orphans a `node --watch` still holding the API port — and the next `apograph dev` fails on a busy port, blaming a process the user cannot see.
...
| ---------------------------- | --------------------------------- | -------------------------------------- |
| apograph build                  | yes                               | yes, if `apps/admin/index.html` exists |
| apograph build --server         | yes                               | no                                     |
| apograph build --admin          | no                                | yes                                    |
| apograph build --server --admin | refused: it asks for neither half |                                        |

The last row used to be a consequence of the check order rather than a design — `adminOnly` skipped `tsc`, then `serverOnly` returned early before Vite, so both flags together gave a command that did nothing and exited zero. `halves()` now refuses the pair for `dev` and `build` alike. The same two flags narrow `apograph dev` to one half.
...
The shortest command in the package: check that `dist/server/src/main.js` exists, and run it. No building — if the file is missing, that is a refusal reading ``… does not exist — run `apograph build` first.`` Production is literally `apograph build && apograph start`: one process, one origin, the API and the admin UI together.

### 3.5 `apograph migrate`
...
```
Applying migrations for 12 plugin(s) to localhost:5432/apograph_cms
...
```
apograph generate --name=add_content_types
...
2. **`npm run migrate` → `apograph migrate`.** First `findProjectRoot()` walks up from the current directory to the nearest `package.json`. Then `loadEnv(root)` reads `.env` — and that happens **before** any command, so that `apograph.config.ts` finds its settings and the child processes inherit the variables.
...
4. **The compiled host is read.** `require('dist/server/apograph.config.js').default` and `require('dist/server/src/plugins.js').buildPlugins(config)`. Both absences and both "wrong export shapes" have messages of their own.
...
7. **`npm run dev` → `apograph dev`.** The API on `:3000`, the admin UI on `:4200`.
8. **The first sign-in.** The administrator's account is provisioned at startup from `APOGRAPH_ROOT_ADMIN_EMAIL` / `APOGRAPH_ROOT_ADMIN_PASSWORD` — that is the identity plugin's doing, not the CLI's. But it is `loadEnv` that gets the variables to it.

### 4.2 Applying migrations after a package update

1. **The `@apograph/*` versions are raised in lockstep.** The packages are released as one set; a partial update can leave two copies of a shared package in `node_modules`.
2. **`apograph migrate`.** Rebuilding the server here is not a formality: **a stale `dist/` would silently migrate against the previous composition** — a plugin added an hour ago would simply not get its tables, and nobody would say so.
...
2. **Pass them into `ContentPlugin({ types, migrations })`** in `apps/server/src/plugins.ts`. The `migrations` descriptor is what `apograph migrate` will latch onto.
3. **Add an `apps/server/drizzle.config.ts`** with `schema` and `out` paths. Without it `apograph generate` refuses to run and explains what is missing.
   _paths in the config resolve against the cwd rather than against the config itself — see section 11_
4. **`apograph generate --name=add_content_types`.** drizzle-kit diffs the schema against the snapshot in `migrations/meta/` and writes the SQL. **No database is required** — which is why there are no secrets in the config.
5. **`apograph migrate`.** The generated SQL is applied through the same shared loop as every plugin's migrations.
6. **The SQL is committed into the application's repository.** Just as plugins commit theirs.

### 4.4 A production build and start

1. **`apograph build`.** `tsc` compiles the server into `dist/server` and Vite builds the admin UI into `dist/admin`. **Nothing is bundled.**
2. **`apograph migrate` in the deployment window.** As a separate step, before the new process starts — migrations are not tied to the application booting.
3. **`apograph start`.** A check that `dist/server/src/main.js` exists, then `node` on it. The process serves the API and, through the config's `staticDir`, the built admin UI — **one process, one origin**.
...
| Aspect        | `@apograph/nx` (the monorepo)                                          | `@apograph/cli` (an installed application)                                      |
| ------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| What is read  | `apps/server/apograph.config.ts` and `src/plugins.ts` — **TypeScript**    | `dist/server/apograph.config.js` and `dist/server/src/plugins.js` — **JavaScript** |
...
Why "just jiti" is not enough for the monorepo: loading `apograph.config.ts` or `buildPlugins` drags in the whole plugin graph — NestJS modules and their DTOs — and that graph uses **legacy decorators** (`experimentalDecorators`). The babel built into jiti ignores the repository's tsconfig and defaults to stage-3 decorator semantics: a field with a definite assignment under a decorator (`email!: string`) gets an initializer, and `transform-typescript` fails. So `@apograph/nx` holds `createTsJiti` — jiti with an swc transform in `legacyDecorator` + `decoratorMetadata` mode, so that the same source is read the way the application itself compiles it.

> **What follows from this**
>
> Three consequences, all three in the consumer's favour. **First:** `jiti` and `@swc/core` stay dependencies of `@apograph/nx` and never reach an installed application's tree. **Second:** a whole class of "the config is read by one compiler while the application is built by another" divergences disappears — what is read is exactly the JavaScript that will later run. **Third:** a stale `dist/` stops being a possible source of a silent error, because the command builds it itself.
...
| Check                            | Message                                                                                                         |
| -------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| No `dist/server/apograph.config.js` | … does not exist — the app has not been built. Run \`apograph build\` first (\`apograph migrate\` does this for you). |
| No `dist/server/src/plugins.js`  | the same message with a different path                                                                          |
| The config has no default export | … has no default export — apograph.config.ts must \`export default\` the app's config.                             |
...
`LAYOUT` in `src/lib/project.ts` is the CLI's entire “config”: `apps/server/tsconfig.json`, `apps/admin/vite.config.mts`, `apps/admin/index.html`, `dist/server`, `dist/admin`, `dist/server/src/main.js`, `dist/server/apograph.config.js`, `dist/server/src/plugins.js`, `apps/server/drizzle.config.ts`. The layout mirrors Apograph's own `apps/` folder (`server`, `admin`, `server-e2e`, `admin-e2e`), so that someone who has read the Apograph sources finds the same shape in their own project.

The compiled paths follow from `apps/server/tsconfig.json` setting `rootDir: "."` and `outDir: "../../dist/server"`: `apps/server/apograph.config.ts` → `dist/server/apograph.config.js`, `apps/server/src/main.ts` → `dist/server/src/main.js`. Change `rootDir` without changing `LAYOUT` and the paths stop resolving: `apograph start` will then report a missing entry point rather than an incorrect setting.
...
>
> Without cleanup, quitting `apograph dev` leaves orphaned `tsc --watch` and `node --watch` processes behind, and the second one keeps holding the API port. The next `apograph dev` fails with “port in use”, and the culprit is a process the user cannot see and did not start in this session. Debugging that costs incomparably more time than the handler itself is worth.
...
| DATABASE_URL          | indirectly: through `config.database.url` in `apograph.config.ts`                      | The only variable whose absence the CLI turns into a refusal of its own (`requireDatabaseUrl`)   |
...
>
> In a generated application, `apps/server/apograph.config.ts` is declared the **single place that reads `process.env`**: everything downstream — the host and every plugin — receives already-typed values. The CLI upholds that rule rather than breaking it: it does not parse `DATABASE_URL` itself, it takes `config.database?.url` from the loaded config. That is why “where did this setting come from” has exactly one answer.

### 6.2 What the application owner sets (the `.env` template)

These variables are read by `apograph.config.ts` and the plugins, but it is `loadEnv` that carries them into the process. The list depends on which capabilities were chosen in the `create-apograph-app` wizard; below are the ones that are always there.

| Variable                            | Purpose                                                                                                                         | Required?                                                                                |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| DATABASE_URL                        | PostgreSQL connection string                                                                                                    | **Required.** Without it `migrate` and `studio` refuse to run                            |
| PORT                                | The API port; `apograph start` serves the admin UI from the same one                                                               | Defaults to 3000                                                                         |
...
| APOGRAPH_ROOT_ADMIN_EMAIL / \_PASSWORD | Provisioning the first administrator account at startup                                                                         | Needed for the first sign-in, then cleared                                               |
...
`@apograph/cli` lands in a new application not as a choice but as a constant: it is listed in the generator's `CORE_DEV_PACKAGES` and is always written into `devDependencies` with the version stamped in by `create-apograph-app` itself. The `package.json` template wraps six commands in scripts right away, so the application owner usually interacts with the CLI only through `npm run`.

```
"scripts": {
    "dev": "apograph dev",
    "build": "apograph build",
    "start": "apograph start",
    "migrate": "apograph migrate",
    "generate": "apograph generate",
    "studio": "apograph studio",
    …
}
```

There is a seventh consumer too, already inside the application itself: the `apps/server-e2e` suite creates and migrates its own `<database>_e2e` database with the **same** `applyPluginMigrations` function that `apograph migrate` calls. That means the schema the e2e tests run against is the real one, not a separately maintained copy.
...
| ----------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- |
| The command was run outside an application                                    | No package.json in /home/user or any parent directory — run this inside an Apograph app.                                                                                                                                   | findProjectRoot            |
| The application is not built and `start` was run                              | dist/server/src/main.js does not exist — run \`apograph build\` first.                                                                                                                                                     | commands/start             |
| The application is not built and `studio` was run with someone else's `dist/` | … does not exist — the app has not been built. Run \`apograph build\` first (\`apograph migrate\` does this for you).                                                                                                         | loadHost                   |
| A TypeScript compilation error                                                | The full `tsc` output (stdio is inherited), then one line: `tsc exited with code 2`                                                                                                                                     | run()                      |
| `apograph.config.ts` without `export default`                                    | dist/server/apograph.config.js has no default export — apograph.config.ts must \`export default\` the app's config.                                                                                                           | loadHost                   |
...
| `apograph studio --host 0.0.0.0`                                                 | A multi-line warning about the absence of authentication and full write access — **before** Studio starts. Not a refusal: the command carries on                                                                        | warnIfExposed              |
...
The wording is “action → expected result”, so items can go into a test case without rewriting. Existing automated coverage: **130** unit tests in fourteen files next to the code, mocking at the process boundary (`node:child_process`, `pg`), so the package is tested without a database. Run them with `npx nx test @apograph/cli`. The gap this dossier first recorded — `cli.ts`, `project.ts`, `env.ts`, `run.ts` and all six commands untested — **is closed**: the dispatcher, the argv readers, the root walk, the `.env` load, the child-process helper and every command now have a spec beside them. What is still checked only by hand is this list against a real application.

### Argument parsing and help

- **`apograph` with no arguments** → prints help, exit code 0.
- **`apograph build --help`** → prints help and builds **nothing**, code 0.
- **`apograph --help`, `apograph -h` and `apograph help`** → prints help, exit code 0, and without going looking for an application first. `wantsHelp` reads the **whole** argv rather than the arguments after the command, which is what keeps the flag out of the command position; the three forms and the precedence against `--version` are pinned by `args.spec.ts`.
- **`apograph buld`** → `Unknown command "buld".`, help, code 1.
- **`apograph generate --name add_x` (a space instead of `=`)** → the name is recognised exactly as in the `--name=add_x` form.
- **`apograph generate --name --other`** → the name is not picked up (the value starts with `-`), drizzle-kit chooses a name itself.

### Finding the root and the environment

- **`apograph migrate` from a subdirectory of the application** → works exactly as from the root; relative paths mean the same thing.
- **Running outside any application** → the “run this inside an Apograph app” message, code 1.
...
- **`apograph build` in a clean application** → `dist/server/` and `dist/admin/` appear; `node_modules` is not inlined into the bundle.
- **`apograph build --server`** → Vite does not run at all.
- **`apograph build --admin`** → `tsc` does not run; `dist/server` stays as it was.
- **`apograph build` in an application without `apps/admin/index.html`** → the “skipping the admin build” line, exit code 0.
...
- **`apograph start` without building first** → “dist/server/src/main.js does not exist — run \`apograph build\` first.”, code 1.
- **`apograph start` after a build** → one process serves both the API and the admin UI from a single origin; Ctrl+C gives code 0.

### dev

- **`apograph dev` with an empty `dist/`** → the server comes up (rather than “hanging”): the one-shot build ran before `node --watch`.
- **Editing a server file** → `tsc` recompiles, `node --watch` restarts; the Nest logs are not wiped off the screen.
- **Ctrl+C, then immediately `apograph dev` again** → the second run does not fail on a busy port — no orphaned processes were left.
...
- **`apograph.config.ts` without `export default`** → the message about the missing default export, not `Cannot read properties of undefined`.
...
- **`apograph studio`** → Studio on `127.0.0.1:4983`; every applied table is visible.
...
- **`npx nx run server:db:migrate` and `apograph migrate` on the same plugin list** → the same order, the same history tables, the same refusal text.
- **`npx nx run <plugin>:db:generate` and `apograph generate`** → both call `drizzle-kit` the same way, differing only in `cwd` and the config path.
- **The built package tarball** → `bin.apograph` points at an existing file in `dist/`, not at `src/cli.ts`.

<details>
<summary>How the package's existing unit tests are put together</summary>

The specs live **next to** the code (`src/lib/*.spec.ts`) and mock strictly at the process boundary: `node:child_process` for `generate` and `studio`, `pg` plus `drizzle-orm` for `migrate`. That is why `npx nx test @apograph/cli` requires neither a database nor a built application.
...
## 10. Boundaries: the CLI and @apograph/nx
...
| Trait                               | @apograph/cli                                                     | @apograph/nx                                                                             |
| ----------------------------------- | ----------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| Who it is for                       | An application installed from npm                                 | The Apograph monorepo                                                                    |
| Published                           | yes, to npm                                                       | no (`private: true`)                                                                     |
| How it is invoked                   | the `apograph` binary / `npm run`                                    | Nx targets (`db:generate`, `db:migrate`, `db:studio`)                                    |
| How the targets appear              | a hard-coded list of six commands                                 | inference (`createNodesV2`) from the presence of `drizzle.config.ts` / `apograph.config.ts` |
...
| Reading `process.env`               | `apps/server/apograph.config.ts`                  | Carries the variables into the process and takes `config.database.url`                |
| The runtime database connection     | `@apograph/database`                           | Opens **its own** temporary pool only for the duration of the migration and closes it |
...
| Creating an application             | `create-apograph-app`                             | Nothing; it lands in the application as a `devDependency`                             |
...
- **An `apograph migrate --dry-run` command** — or any way at all to see what would be applied without applying it.
...
| ------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| packages/cli/AGENTS.md, the command table                                | “`apograph start` — Runs `dist/server/main.js`”                                                                           | `LAYOUT.serverEntry` = `dist/server/src/main.js`. The `src/` segment is preserved because the template's `apps/server/tsconfig.json` has `rootDir` equal to `"."`, not `"src"`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| packages/cli/AGENTS.md, “Compile first”                                  | “`require`s `dist/server/{apograph.config,plugins}.js`”                                                                   | The paths differ: `dist/server/apograph.config.js`, but `dist/server/src/plugins.js`. The brace form implies a shared directory that does not exist                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| packages/cli/src/cli.ts, the Options block                               | “`--server` — build/**dev**: the server only, skipping the admin”                                                      | **Fixed.** `devCommand(root)` took no options at all and did not read argv, so `apograph dev --server` still brought Vite up. Both flags now work for `dev` as well — `--admin` runs the Vite dev server alone against an API already up, the way the monorepo offers `start:server` / `start:admin` — and `--admin` asked of an application with no `apps/admin/index.html` is a refusal rather than a supervised empty set exiting 0. `cli.spec.ts` and `commands.spec.ts` carry the regression                                                                                                                                                                                                                                                                                                    |
| packages/cli/src/cli.ts, the Options block                               | “`-h, --help` — Show this message”                                                                                     | **Fixed.** It used to be true — the first argv element was always treated as a command, so `apograph --help` and `apograph -h` answered `Unknown command` with exit code 1, and help was reachable only as `apograph` with no arguments or `apograph <command> --help`. `wantsHelp` now reads the whole argv, accepts a bare `help` as well, and is checked before the project root is looked for; `args.spec.ts` carries the regression                                                                                                                                                                                                                                                                                                                                                                      |
| packages/cli/src/lib/studio.ts                                           | The `--host`/`--port` options are described as passed straight through to drizzle-kit                                  | **Fixed.** `--port=0` was **silently dropped** (`0` is falsy in `if (options.port)`) and Studio came up on 4983, while `@apograph/nx`'s `db-studio` executor refused the same input with an explanation — **two branches of one tool behaving differently**. The refusal now lives in `runDrizzleKitStudio`, the implementation both worlds call, and it is a refusal rather than a pass-through for the reason that executor gave: drizzle-kit's listen callback is handed the bound address and ignores it, printing the port it was _asked_ for, so `--port=0` would leave an unauthenticated read/write console on the database at an address nothing reports. `--port=abc` is reported by the argv reader instead of becoming `NaN` and vanishing                                            |
| create-apograph-app · README.md.tmpl, “Adding content types”, step 3        | “Add `apps/server/drizzle.config.ts` pointing `schema` at your `src/content/index.ts` and `out` at `../../migrations`” | The paths are written as if they resolved relative to the config file. But `generateCommand` starts drizzle-kit with `cwd` = **the application root**, and drizzle-kit resolves `schema`/`out` relative to the working directory (this is stated outright in the comment in `src/lib/generate.ts`). Following the instruction literally sent `out` two levels **above** the application root, and `schema` was not found. **Fixed:** the step is now the config itself, with `out: 'migrations'` and the full `apps/server/src/content/index.ts`, plus the sentence saying both are relative to the application root and why. `generated-readme.spec.ts` resolves `out` from the root the CLI runs drizzle-kit in and compares it with the directory the plugin's own migrations descriptor names |
| create-apograph-app · templates/default/apps/server/src/plugins.ts, comment | “Define some in `src/server/content/`”                                                                                 | **Fixed.** No such path existed in the template — the server side lives in `apps/server/src/` — while the same template's README named the correct one                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| create-apograph-app · templates/default/apps/server/src/plugins.ts, example | `migrations: { dir: () => join(process.cwd(), 'migrations') }`                                                         | Depends on the current directory, whereas `findProjectRoot` exists precisely so a command can be run from a subdirectory. The monorepo uses `join(__dirname, '../migrations')` in the equivalent place, and every plugin uses `join(__dirname, '…')`. The template's example breaks in exactly the scenario the CLI promises to support                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| create-apograph-app · README.md.tmpl, the command table                     | `npm run generate -- --name=<name>` was listed alongside the rest as ready to use                                      | A freshly created application has no `apps/server/drizzle.config.ts` (the template does not contain that file), so the very first run is a refusal. This is intended and explained in “Adding content types”, but nothing marks it in the table                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| packages/cli/AGENTS.md, structure                                        | The “## Commands” heading appears **twice**: once as the table of `apograph` commands, once as a list of `npm exec nx …`  | The second occurrence is about developing the package itself. The duplicate heading breaks navigation through the document                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
The source is the package's own code: `src/cli.ts`, `src/index.ts`, six commands in `src/lib/commands/`, six core modules in `src/lib/` and the fourteen spec files beside them. Cross-checked against the `@apograph/nx` executors (`db-migrate`, `db-generate`, `db-studio`, `lib/jiti.ts`), against the `create-apograph-app` template (`package.json.tmpl`, `env.tmpl`, `README.md.tmpl`, `apps/server/{tsconfig.json,src/plugins.ts}`) and against the `ServerPlugin` type from `@apograph/bootstrap-server`. The `AGENTS.md` files were used as a skeleton, but every statement was verified against the implementation — the divergences are collected in section 11.
