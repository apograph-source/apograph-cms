...
Besides tools the endpoint serves **resources** — documents a client can pull into its context without spending a tool call. Today there is exactly one kind: a content type's schema at the URI `orthacms://content-type/<name>`, `mimeType: application/json`.
...
| name           | —                    | `'orthacms'`       | The server's identity in `initialize` — what the client shows in its connector list. A literal rather than a variable: this is stable product configuration. An empty value is an exception at plugin construction                                                                                               |
...
| .env.example (the repository root)                                              | Only `MCP_ENABLED` is documented                                                                                                                      | `MCP_CALL_TIMEOUT_MS` and `MCP_MAX_RESULT_BYTES` are **absent** from the file, even though the package's `AGENTS.md` names them and `orthacms.config.ts` reads them. Meanwhile the scaffolder's template (`packages/create-orthacms-app/templates/default/env.tmpl`) contains both variables — so a generated application is documented better than the repository itself                                                               |
