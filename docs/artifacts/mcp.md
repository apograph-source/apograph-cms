...
- **It is not the tool registry.** `ToolDefinition`, `ToolContext`, `ToolProvider`, `ToolRegistry`, `createToolContext` and `toToolError` live in `@apograph/tools-server`. MCP _imports_ `ToolsModule` rather than providing it — otherwise a deployment that only wants the copilot would drag `@modelcontextprotocol/sdk` in through this barrel, that is, a dependency on a protocol it does not speak.
...
| --------------------------- | ----------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| @apograph/tools-server      | `ToolRegistry`, `ToolsModule`, `ToolContext`, `createToolContext`, `toToolError`, `ToolOutput`, `ToolError` | The tool catalogue and the **single** point where a call is authorized             |
| @apograph/identity-server   | `ApiTokenService.verify`, `scopePermissions`, the `@Public()` decorator                                     | The credentials and the translation of a token scope into a permission set         |
| @apograph/workspaces-server | `WORKSPACE_HEADER`, `WORKSPACE_ID_PATTERN`                                                                  | The header name and the identifier format — the same constants the REST guard uses |
| @apograph/bootstrap-server  | The `ServerPlugin` type                                                                                     | The plugin contract for the host                                                   |
...
Besides tools the endpoint serves **resources** — documents a client can pull into its context without spending a tool call. Today there is exactly one kind: a content type's schema at the URI `apograph://content-type/<name>`, `mimeType: application/json`.
...
| Content revisions and the outbox | `content-server` + `@apograph/database`                                    | A handler calls the same `EntryWriterService` the HTTP controller does — and gets the same numbering under an advisory lock and the same outbox write |
...
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Issue an agent its access                           | The **“API tokens”** page (`@apograph/api-tokens-admin`; the server half is inside `identity-server`) | The scope — `read` for a reading agent, `full` for a writing one. That is what decides which tools the agent will see at all. Plus an explicit list of workspaces |
...
| name           | —                    | `'apograph-cms'`       | The server's identity in `initialize` — what the client shows in its connector list. A literal rather than a variable: this is stable product configuration. An empty value is an exception at plugin construction                                                                                               |
...
| The database connection and migrations                                          | `@apograph/database` + `@apograph/nx`                                               | Nothing. No tables, no migrations, no schema                                                                  |
...
| .env.example (the repository root)                                              | Only `MCP_ENABLED` is documented                                                                                                                      | `MCP_CALL_TIMEOUT_MS` and `MCP_MAX_RESULT_BYTES` are **absent** from the file, even though the package's `AGENTS.md` names them and `apograph.config.ts` reads them. Meanwhile the scaffolder's template (`packages/create-apograph-app/templates/default/env.tmpl`) contains both variables — so a generated application is documented better than the repository itself                                                               |
