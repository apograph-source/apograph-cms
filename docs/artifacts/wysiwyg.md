...
- **It is not a field type.** `richtext` exists in `@apograph/content-domain` and `@apograph/content-server` independently; the plugin changes the _control_, not the schema, not the validation and not the column.
- **It is not the media library.** Browsing folders, filtering by type and uploading are entirely the work of `@apograph/media-admin`, which arrives into the declared slot from outside.
...
| -------------------------------------------- | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| domain/constants                             | domain       | A pure dictionary: `WYSIWYG_WIDGET`, `WYSIWYG_MEDIA_KIND`, `MEDIA_ALIGN`, `CALLOUT_TONE`, `COLUMN_COUNTS`, the `TEXT_COLORS`/`HIGHLIGHT_COLORS`/`FONT_SIZES` palettes, the `apograph-wysiwyg` class, and the `MEDIA_MIN_WIDTH = 64` and `MEDIA_RESIZE_STEP = 16` thresholds |
...
| infrastructure/extensions/tableTab           | infra        | The `apographTableTab` extension (`priority: 200`) — constrains Tab at the end of a table                                                                                                                                                                                   |
...
| src/styles.css                               | —            | 429 lines, all hanging off `.apograph-wysiwyg`; the host imports it once after the design system                                                                                                                                                                            |
...
| wysiwyg.media.sources<br>WYSIWYG_MEDIA_SLOT            | `declares` | Media sources. `@apograph/media-admin` puts two in: `media.wysiwyg.library` ("Media Library…", `order: 10`) and `media.wysiwyg.upload` ("Upload files…", `order: 20`). With not one contribution the editor still inserts media — only the built-in "by URL" items remain |
...
| **Tab at the end of a table**                                      | An override of TableKit's behaviour (`apographTableTab`, `priority: 200`)                                                                                                               | One new row — **once**. A second Tab falls through the just-created empty row and focus leaves the document                                                                                                                                                                     | Tab                                                                                   |
...
The editor can **hold** an image and a video, and knows **one** way of naming them — pasting a URL. It knows nothing about the media library, and that is a decision: importing `media-admin` would make rich text unusable in an installation with no media plugin and would nail the editor forever to one particular library's shape. So the editor _declares_ `WYSIWYG_MEDIA_SLOT` and `@apograph/media-admin` fills it — the same inversion `content-admin` uses for its own slots, and the reason the dependency runs media → wysiwyg rather than the other way.
...
| ⌘K / Ctrl+K        | The document             | **Suppressed** while the caret is in the body — but not by this package: the shared `isComposingText` helper from `@apograph/utils-admin` closes global chords over `input`, `textarea`, `select` and any `contenteditable`. Opening the palette from under the caret is a change of context in response to typing in _another_ control (3.2.2), and ⌘K in any editor means "insert a link" |
...
- **I-34** — Every style rule in the package hangs off `.apograph-wysiwyg`: none can reach the rest of the admin UI, and the editor surface and the preview render the same markup identically.
