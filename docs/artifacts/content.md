...
**The content model, the editor and the public API — Apograph's heart**
...
- **It is not a rich-text editor.** The kernel describes `richtext` as a structured document and validates it (heading order, table headers, link text, language markers), but the TipTap editor itself comes from `@apograph/wysiwyg-admin` through a slot.
...
| ------- | ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| domain  | @apograph/content-domain  | The shared kernel — plain TypeScript both runtimes apply identically                                                                                     | The `draft ↔ published` state machine, field value validation, the publication gate, the `richtext` document model and its structural rules, BCP-47. **Zero dependencies** in its `package.json` — verified |
| server  | @apograph/content-server  | The NestJS plugin: the type DSL, the registry, the table generator, the entries engine, versions, the public API, the agent tools, insights, saved views | 2 tables of its own (`saved_views`, `saved_view_defaults`), 61 HTTP routes, 6 extension ports, 24 tools, OpenAPI generation                                                                                  |
| admin   | @apograph/content-admin   | The content library in the admin UI: navigation, the records table, the entry editor, history, saved views, insights widgets                             | The `/workspaces/:id/content/*` route, 14 extension slots, 8 widgets on the Insights page                                                                                                                    |
| graphql | @apograph/content-graphql | A second **protocol** over the same public API (`POST /api/v1/graphql`) — an adapter, not a second API (ADR-0008)                                        | 3 routes, schema assembly per the workspace's grant set, a query cost budget, GraphiQL. **No tables, no migrations, no new way of authenticating**                                                           |
...
>
> Collection files and the schema entry for drizzle-kit must import from `@apograph/content-server/define`, not from the main barrel. drizzle-kit assembles the schema's whole import graph with plain esbuild, which trips over the NestJS decorators arriving through the main barrel's controllers. `/define` re-exports only the decorator-free DSL: `collection`, `single`, `field`, `joinTableOf`, the types and the `content_entry_revisions` table.

> **Neighbours that are easy to confuse**
>
> **`@apograph/i18n-server`** binds `CONTENT_ENTRY_EXTENSION`: locales, shared-field synchronization onto siblings, the `hasLocale`/`missingLocale`/`localeCount` virtual filters. **`@apograph/media-server`** binds `MEDIA_ASSET_RESOLVER`. **`@apograph/segments-server`** registers `CONTENT_READ_SCOPE`, `ENTRY_WRITE_EXTENSION` (the `access` key) and `ENTRY_FILTER_PROVIDER`. **`@apograph/alarms-server`** uses the exported `EntryMatchQuery`. **`@apograph/transfer-server`** handles import/export and writes only through `EntryWriterService`. **`@apograph/mcp-server`** owns the MCP protocol but not the tools; the tool registry is in `@apograph/tools-server`.
...
1. **The author writes the declaration.** `collection('article', { label, publishable, paranoid, i18n, fields: {…} })`. The import is strictly from `@apograph/content-server/define`.
...
5. **`nx run server:db:generate --name=…`** assembles the SQL and the developer commits it. For the package's own tables the command differs: `nx run "@apograph/content-server:db:generate"`.
...
4. **The model learns a type's shape on demand** through `content_type_get`, whose `valuesSchema` comes from the same `docs/field-schema.ts` as the OpenAPI document. Granted types are additionally available as MCP **resources** at `apograph://content-type/<name>` — through the same grant gate.
...
- **The properties panel is short on purpose.** The publication gate, Details, and whatever a plugin contributes — and nothing that restates what a tab already owns. A truncated Revisions block used to sit here beside a History tab showing the whole timeline with the same Restore, Publish and Preview actions: a second copy of the same list, with a second set of loading and error states to keep honest, and the tallest thing in a panel whose job is properties. It was removed (`ORT-227`), and the locale panel with it — the locale switcher is the title-row chip's menu now, and the translation-group id is one row of **Details**, contributed through `ENTRY_DETAILS_ROW_SLOT`.
- **The publication gate lists what is failing, not what exists.** Passing checks were a re-statement of every required field on the type, which the reader had to scan to find the one red line; with nothing failing the gate is a single sentence (“Every check passes — ready to publish.”) and the heading reads `ready`, and the moment something blocks the list is exactly the outstanding work under a `{n} blocking` heading. **Rendering only** — what fails is still decided once, in the kernel (`content:I-38`); this component counts and draws. Not collapsible, and not to become so: the verdict decides what is shown, so there is no control and no remembered state to fall out of step with the entry. It is built from the **visible** fields — hidden ones (`admin.hidden`), ungranted relations and link-managed relations go into `validationIgnored`. Otherwise a hidden required field showed “ready” in the panel while validation still counted it and a toast named it — a control the user will never find.
...
- **The key roots are `content-entries` / `content-entry` / …, never `content`.** TanStack matches _segments_, so the obvious `invalidateQueries({ queryKey: ['content'] })` matches **nothing**: `'content' !== 'content-entries'`. It throws nothing and returns no count — the mutation succeeds, the toast appears, the table does not move. `@apograph/transfer-admin` shipped exactly that after an import. A plugin that writes entries must call the exported `refreshEntryCaches` rather than invent its own key.
...
| ENTRY_SIDEBAR_WIDGET_SLOT  | A **section** of the properties panel (render the exported `EntrySidebarSection`/`EntrySidebarRow`, not a card)                                                                                                                                                                              | alarms (Checks), protection (Review)                     |
| ENTRY_DETAILS_ROW_SLOT     | One **row** of the Details block, after the built-in ones. The component must render an `EntrySidebarRow` (a `<dt>`/`<dd>` pair) or `null` — the rows land inside Details' own `<dl>`. `order` is load-bearing: `getItems()` does not sort, so the render site does                          | i18n (the translation-group id)                          |
| ENTRY_HEADER_SLOT          | An inline element in the entry's title row, after the `<h1>`                                                                                                                                                                                                                                 | i18n (the locale chip, which is also the locale menu's trigger), segments (the “restricted” chip) |
...
| ENTRY_PUBLISH_GUARD_SLOT   | The **third gate** on the editor's Publish, the admin counterpart of the server's `CONTENT_PUBLISH_GUARD`. `useVerdict(context, { dirty })` returns `{ blocked, reason?, action?, overlay? }`, or `null` for no opinion; the **first refusal wins** and content never learns what refused it                                 | protection (the approval rule)                           |
...
- **I-37** — The `@apograph/content-domain` kernel declares no dependencies; the `entries/domain/` layer imports neither NestJS, nor Drizzle, nor class-validator, nor its own infrastructure (checked by grep).
...
| ----------------------------------------------------------- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| The database connection and running migrations              | `@apograph/database` + `@apograph/nx` | Owns the `saved_views` schema files, but not the connection and not the application step                                                         |
...
| packages/content/server/AGENTS.md, the “Agent tools” section  | “contributed to the shared **tool registry** in `@apograph/mcp-server`”                                                                      | The registry lives in `@apograph/tools-server` (ADR-0007), and the provider imports exactly that: `import { … } from '@apograph/tools-server'`. `@apograph/mcp-server` owns only the protocol. The root `AGENTS.md` gets this right                                                                                                                                                                                                      |
| packages/content/server/package.json                          | `@apograph/mcp-server` in `dependencies`                                                                                                     | There is **not one import** from that package in `src/` (checked by grep across the whole tree). A dead dependency — probably a leftover from the registry's move to `tools-server`; it also drags an extra edge into the build graph                                                                                                                                                                                                    |
