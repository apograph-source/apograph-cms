...
> `reveal-link` — and phase 3's **scaffolder question** with it: `create-orthacms-app`
...
> `AGENTS.md` files are now the reference for their internals, and
> [`docs/artifacts/mail.md`](../artifacts/mail.md) is the dossier. Where the
> implementation diverged from what was planned here, this document has been
> corrected rather than left as a second, wrong account.

## What it is

One **mail provider** per deployment, a queue in front of it, and the messages
the CMS actually needs to send — three today, a fourth with the phase-2 recovery
route. It exists because the product used to send nothing: an invitation and a
reset link were returned to the administrator, who relayed them by hand, and
self-service password recovery therefore did not exist at all.
...
export interface MailMessage {
    to: string;
    from: string; // the configured sender, carried on the message
    replyTo?: string;
    subject: string;
...
| ----------------- | ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `id`              | `uuid` pk                    |                                                                                                                                                                                                                                                                                                       |
| `kind`            | `text` not null              | `invite` · `invite_resent` · `password_reset`. What it was, for the dead-letter surface. `password_recovery` joins them with the phase-2 route, not before: a kind nothing can produce is a template and a branch nobody executes.                                                                    |
| `to_address`      | `text` not null              |                                                                                                                                                                                                                                                                                                       |
| `subject`         | `text` not null              |                                                                                                                                                                                                                                                                                                       |
| `body_text`       | `text` not null              | **Rendered, link included.**                                                                                                                                                                                                                                                                          |
| `body_html`       | `text`                       |                                                                                                                                                                                                                                                                                                       |
| `link`            | `text`                       | **Added while building it.** The link on its own, so `reveal-link` hands the secret back without parsing a rendered body — which would make the copy and a security-relevant route load-bearing on each other. The same secret the body already carries, in the same row, deleted at the same moment. |
...
| `dead_at`         | `timestamptz`                | **Added while building it.** When the message was given up on. The plan said a dead letter is a row at the attempt cap, but I-08 stops a permanently rejected row on its _first_ attempt with its budget untouched, and both have to read as dead letters without faking the count.                   |
| `last_error`      | `text`                       |                                                                                                                                                                                                                                                                                                       |
...
There is no second table. A dead letter is a row with `dead_at` set — the
attempt cap reached, or a permanent rejection — which is what the surface
reads.
...
- Claim with `FOR UPDATE SKIP LOCKED`, ordered by `next_attempt_at`, in a short
  transaction that commits before any send. The claim **leases** the row by
  pushing `next_attempt_at` forward by `claimLeaseMs`, so a second worker skips
  a row that is in flight and a process that dies mid-send releases it by
  timeout — which is why there is no `delivering` status and no reaper.
...
| `POST /api/users/:id/reveal-link`  | `users:manage`  | Returns the raw link of the newest **undelivered** message, for when one did not arrive. Audited. The only way to see a secret once a provider is configured — and only ever an undelivered one, because a delivered row is deleted with its link. `409` on a deployment that sends no mail, and on a member with nothing outstanding. |
...
- **I-13** `console` and `testkit` are registered by no template and offered by
  no picker — pinned on the scaffolder side by `create-orthacms-app:I-35`.
...
1. **Messages go out** — ADR accepted, `mail/domain`, `mail/server` with the
   ...
   **Done.** `users:manage` was added to the permission catalogue for
   ...
   `create-orthacms-app` until the phase-3 question existed; they are now split
...
3. **Seams** — the `create-orthacms-app` question (**done**: the fifth question,
