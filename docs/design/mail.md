...
> `reveal-link`. Phases 2 and 3 below are not. The decision behind it is
...
1. **Messages go out** — ADR accepted, `mail/domain`, `mail/server` with the
   ...
   **Done.** `users:manage` was added to the permission catalogue for
   reveal-link, and the five packages are classified as transitive in
   `create-apograph-app` until the phase-3 question exists to make them a
   feature group.
