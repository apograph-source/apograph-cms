...
> (`@ortha/copilot-*`), the routes (`/api/copilot/runs`), the permission
...
| Own tables & migrations         | ready      | `@ortha/nx` infers `db:generate` from a `drizzle.config.ts`; the host's `db:migrate` reads `plugins.ts`.                                                                                                       |
