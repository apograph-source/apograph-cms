...
gets an Ortha CMS account with an Ortha CMS role; what changes is who checks the
credential.
...
That last point is what keeps phase 0 small and safe. Ortha CMS is invite-only by
...
| ------------------- | ------------------------------------- | ---------------------------------------------------------- |
| `sso_identities`    | Links an Ortha CMS user to an IdP subject | unique `(provider, subject)`; unique `(provider, user_id)` |
...
| Offboarding                    | Say plainly that a disabled IdP user keeps their Ortha CMS session for up to `SESSION_TTL_SECONDS`. Offer a shorter TTL for SSO sessions; back-channel logout is phase 3. | Operators buy SSO expecting instant offboarding. Left unsaid, this is a security surprise rather than a documented limit.                          |
...
- `create-orthacms-app/src/lib/features.ts` must classify every new package, and
...
- `orthacms.config.ts` gains an `sso` block under `plugins.identity`, with each
