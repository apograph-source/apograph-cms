 ...
 *
 *   slot 0 (the main checkout)  PORT=3000  ADMIN_PORT=4200  db orthacms
 *   slot 1                      PORT=3001  ADMIN_PORT=4201  db orthacms_a1
 *   slot n                      PORT=300n  ADMIN_PORT=420n  db orthacms_an
 *
...
const BASE_DATABASE = 'orthacms';
/** `container_name` in docker-compose.yml. */
const DEFAULT_CONTAINER = 'orthacms-postgres';
...
function provision({ positionals, flags }) {
    ...
    if (slot === 0) {
        fail(
            'slot 0 is the main checkout — it already owns :3000/:4200 and `orthacms`.'
        );
    }
    ...
    const sourceUrl =
        ...
        'postgresql://orthacms:orthacms@localhost:5432/orthacms';
